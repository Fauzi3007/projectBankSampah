<x-app-layout>
  <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
      ...
      <!-- Cards -->
      <a href="{{route('provinsi.create')}}" class="btn bg-indigo-500 hover:bg-indigo-600 text-white">
          <svg class="w-4 h-4 fill-current opacity-50 shrink-0" viewBox="0 0 16 16">
              <path d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
          </svg>
          <span class="ml-2">Tambah Provinsi</span>
      </a>
      @if(session('success'))
      ...
      <div class="grid grid-cols-12 gap-6 mt-2">
        <div class="col-span-full  bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
            <header class="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                <h2 class="font-semibold text-slate-800 dark:text-slate-100">Provinsi</h2>
            </header>
            <div class="p-3">
                    ...
                    <table class="table-auto w-full">
                        ...
                        <thead class="text-xs font-semibold uppercase text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-700 dark:bg-opacity-50">
                            <tr>
                                ...
                                <th class="p-2 whitespace-nowrap">
                                    <div class="font-semibold text-left">Nama Provinsi</div>
                                </th>
                                <th class="p-2 whitespace-nowrap">
                        ...
                        <tbody class="text-sm divide-y divide-slate-100 dark:divide-slate-700">
                            <tr>
                                ...
                                <td class="p-2 whitespace-nowrap">
                                    <div class="text-left">{{$item->nama_provinsi}}</div>
                                </td>
                                <td class="p-2 whitespace-nowrap flex justify-center items-center gap-1">
                                    <a href="{{route('provinsi.edit',$item->id_provinsi)}}" class="px-4 py-2 rounded-md bg-yellow-300 hover:bg-yellow-400 text-white sm:mt-0">Edit</a>
                                    <form action="{{route('provinsi.destroy',$item->id_provinsi)}}" method="post" class="d-inline">
                                        @method('DELETE')
                                        @csrf
