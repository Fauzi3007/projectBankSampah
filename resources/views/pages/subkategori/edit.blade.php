<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <form action="{{ route('subkategori.update', ['subkategori' => $subkategori->id_subkategori]) }}" method="post"
            ...
            @csrf
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <x-input id="nama_subkategori" type="text" name="nama_subkategori" :value="old('nama_subkategori',$subkategori->nama_subkategori)" required />
                    ...
                    <select name="kategori_id_kategori" id="kategori_id_kategori" class="rounded-md border-gray-300 block w-full">
                        @foreach ($kategoris as $item)
                            <option value="{{ $item->id_kategori }}"
                                {{ $item->id_kategori == $subkategori->id_kategori ? 'selected' : '' }}>
