<x-app-layout>
    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <form action="{{route('pengguna_sarana.update',$pengguna_sarana->id_pengguna_sarana)}}" method="post" class="grid grid-cols-2 gap-6 mt-2">
            @method('PUT')
              @csrf
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">

                <x-label for="nama_admin">{{ __('Nama Admin') }} </x-label>
                <x-input id="nama_admin" type="text" name="nama_admin" :value="old('nama_admin', $pengguna_sarana->nama_pengguna)"
                    required />
