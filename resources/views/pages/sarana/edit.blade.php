...
{{--
                ...
                <select id="provinsi_id_provinsi" name="provinsi_id_provinsi" :value="old('provinsi_id_provinsi')"
                    ...
                    @foreach ($provinsis as $item)
                        <option value="{{ $item->id_provinsi }}" @if($item->id_provinsi == $sarana->provinsi_id_provinsi) selected @endif>{{ $item->nama_provinsi }}</option>
                    @endforeach
                </select>

                <x-label for="no_hp">{{ __('Nomor Telepon') }} </x-label>
                <x-input id="no_hp" type="text" name="no_hp" :value="old('no_hp',$sarana->no_hp)" required />


                <x-label for="nama_admin">{{ __('Nama Admin') }} </x-label>
                <select id="nama_admin" name="nama_admin" :value="old('nama_admin')" required class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    @foreach($users as $item)
                    <option value="{{ $item->id }}" @if($item->id == $sarana->nama_admin) selected @endif>{{ $item->name }}</option>
                    @endforeach
